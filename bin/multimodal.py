...
if __name__ == "__main__":
    ...
    parser.add_option('-i', action="store", dest="train_data", default="data/task_data/task_informative_text_img_agreed_lab_train.tsv", type="string")
    parser.add_option('-v', action="store", dest="val_data", default="data/task_data/task_informative_text_img_agreed_lab_dev.tsv", type="string")
    parser.add_option('-t', action="store", dest="test_data", default="data/task_data/task_informative_text_img_agreed_lab_test.tsv", type="string")
    parser.add_option('-m', action="store", dest="model_file", default="models/info_multimodal_paired_agreed_lab.model", type="string")
    parser.add_option('-o', action="store", dest="outputfile", default="results/info_multimodal_results_cnn_paired_agreed_lab.txt", type="string")
    parser.add_option("-w", "--w2v_checkpoint", action="store", dest="w2v_checkpoint",
                      default="w2v_models/data_w2v_info_paired_agreed_lab.model", type="string")
    parser.add_option("-d", "--log_dir", action="store", dest="log_dir", default="model_log/", type="string")
    # parser.add_option("-l","--log_file", action="store", dest="log_file", default="log/info_multimodal_paired_agreed_lab.log", type="string")
    ...
    parser.add_option("--image_dump", action="store", dest="image_dump", default="data/task_data/images_data.npz", type="string")
    ...
    with open(options.image_dump,'rb') as handle:
        images_npy_data = np.load(handle,allow_pickle=True)
        images_npy_data = images_npy_data['arr_0']
    
    print(type(images_npy_data))
    ...
    #save_object(DataGenerator(train_image_list, train_x, images_npy_data, train_y, **params), 'train_data_generator.pkl')
    #save_object(DataGenerator(dev_image_list, dev_x, images_npy_data, dev_y, **params), 'val_data_generator.pkl')
    ...
    if (options.w2v_checkpoint and file_exist(options.w2v_checkpoint)):
        ...
        word_vec_model_file = "w2v_models\crisisNLP_word_vector.bin"
    ...
    ######## Image text_network #######
    last_layer_output, res50 = resnet_model()
    ...
    merged_network = concatenate(inputs=[last_layer_output, text_network],axis=-1)
    ...
    model = Model(inputs=[res50.input, inputs], outputs=out)
    ...
    print("checkpoint 8: Saved model training")
    b = datetime.datetime.now().replace(microsecond=0)
    print ("time taken:")
    print(b - a)
    ...
    print("checkpoint 9: Testing ends")
    ...
    print("checkpoint 10: Evaluation done")
